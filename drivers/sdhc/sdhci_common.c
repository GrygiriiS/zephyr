...
static int sdhci_wait_data_complete(struct sdhci_common *sdhci_ctx, uint32_t timeout)
{
	...
	events = k_event_wait(&sdhci_ctx->irq_event,
			      SDHCI_INT_XFER_COMPLETE | SDHCI_INT_ERROR | SDHCI_INT_ERROR_MASK,
			      false, wait_time);
				   ...
				   bool is_read)
{
	...
	rdy_mask = SDHCI_INT_ERROR | SDHCI_INT_ERROR_MASK;
	if (is_read) {
				...
				bool end)
{
	...
	cmd = SDHC_ADMA_DESC_VALID | SDHC_ADMA_DESC_ACT_TRAN;
	if (end) {
		cmd |= SDHC_ADMA_DESC_END;
	}
				    ...
				    uintptr_t dma_addr)
{
	...
	desc_count = DIV_ROUND_UP(trans_bytes, SDHC_ADMA_DESC_MAX_LEN);
	desc_dma_addr = z_mem_phys_addr(sdhci_ctx->adma_descs);
	while (--desc_count) {
		sdhc_adma_desc_fill(desc, dma_addr, SDHC_ADMA_DESC_MAX_LEN, false);
		dma_addr += SDHC_ADMA_DESC_MAX_LEN;
		trans_bytes -= SDHC_ADMA_DESC_MAX_LEN;
		desc++;
	}
	...
	sys_cache_data_flush_range(sdhci_ctx->adma_descs,
				   (sdhci_ctx->adma_descs_num * sizeof(struct sdhc_adma_desc)));
#endif

	sys_write32(desc_dma_addr, sdhci_ctx->reg_base + SDHCI_ADMA_ADDRESS);
}
...
static int sdhc_wait_cmd_complete(struct sdhci_common *sdhci_ctx, int32_t timeout)
{
	...
	events = k_event_wait(&sdhci_ctx->irq_event,
			      SDHCI_INT_CMD_COMPLETE | SDHCI_INT_ERROR | SDHCI_INT_ERROR_MASK,
			      false, wait_time);
